...
export class Nuevo {
	...
	constructor(public navCtrl: Router,  params: ActivatedRoute,public servicio: ServicioDatos,
		...
		this.compra=JSON.parse(params.snapshot.paramMap.get("compra"));
		if (this.compra!=Constantes.COMPRA_COMPRA){
			this.compra=Constantes.INDEFINIDO;
			this.animal=JSON.parse(params.snapshot.paramMap.get("animal"));
		}else{
